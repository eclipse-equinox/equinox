									...
									withEnv(["ws=${ws}", "os=${os}", "arch=${arch}", "isMainIshBranch=${isOnMainIshBranch()}"]) {
										sh '''
											...
											if [[ ${PLATFORM} == cocoa.macosx.* ]]; then
												...
												curl --output 'sdk.entitlement' 'https://raw.githubusercontent.com/eclipse-platform/eclipse.platform.releng.aggregator/refs/heads/master/products/eclipse.entitlement'

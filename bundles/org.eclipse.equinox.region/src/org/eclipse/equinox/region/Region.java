...
/**
 * A <i>region</i> is a subset of the bundles of an OSGi framework. A regions is
 * "weakly" isolated from other regions except that is has full visibility of
 * certain (subject to a {@link RegionFilter}) bundles, packages, and services
...
public interface Region {
	...
	/**
	 * Associates a given bundle, which has therefore already been installed, with
	 * this region.
	 * <p>
	 * This method is typically used to associate the system bundle with a region.
	 * Note that the system bundle is not treated specially and in order to be fully
	 * visible in a region, it must either be associated with the region or imported
	 * from another region via a connection.
	 * <p>
	 * If the bundle is already associated with this region, takes no action and
	 * returns normally.
	 * <p>
	 * If the bundle is already associated with another region, throws
	 * BundleException with exception type INVALID_OPERATION.
	 *
	...
	/**
	 * Associates the given bundle id with this region. If the given bundle id is
	 * already associated with this region, this is not an error and there is no
	 * effect.
	 * <p>
	 * If the bundle is already associated with another region, throws
	 * BundleException with exception type INVALID_OPERATION.
	...
	/**
	 * Connects this region to the given head region and associates the given
	 * {@link RegionFilter} with the connection. This region may then, subject to
	 * the region filter, see bundles, packages, and services visible in the head
	 * region.
	 * <p>
	 * If the filter allows the same bundle symbolic name and version as a bundle
	 * already present in this region or a filter connecting this region to a region
	 * other than the tail region, then BundleException with exception type

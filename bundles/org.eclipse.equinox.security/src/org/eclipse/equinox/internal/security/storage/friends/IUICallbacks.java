...
/**
 * Interface used for UI dependency injection. Unlike most places, actions in
 * the secure storage can be initiated independently from UI so we can't expect
 * UI bundle to be originator of an action - or even being activated.
 *
 * As such, an internal extension point internalUI is used to get UI callbacks.
 *
 * This is an internal interface used to facilitate exchange between core and UI
 * portions.
 *

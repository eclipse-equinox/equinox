 ...
 * preferences can be used to persist sensitive information in an encrypted
 * form.
 * <p>
 * Logically, secure preferences combine functionality of a keyring (keystore)
 * and {@link org.osgi.service.prefs.Preferences}.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * Each node can have a map of keys with associated values. For instance, to
 ...
 * <li>Methods that used to throw BackingStoreException will be throwing more
 * detailed StorageException</li>
 * <li>Navigation on preferences tree will return ISecurePreferences, as
 * opposing to Preferences</li>
 * <li>flush() throws IOException</li>
 * <li>sync() method is removed</li>
 * </ul>
 * <p>
 * On the keyring side, when adding a key to the node, you can ask framework to
 * encrypt it. Framework will lazily acquire password from password provider and
 * use it to encrypt all new entries added to the secure preferences tree in
 * this session.
 * </p>
 * <p>
 * It is worthwhile to reiterate that same limitations as
 * {@link org.osgi.service.prefs.Preferences} apply to the node names. One
 * non-trivial limitation is that node names can not contain forward slashes.
 * For convenience, utility methods {@link EncodingUtils#encodeSlashes(String)}
 * and {@link EncodingUtils#decodeSlashes(String)} are provided to work around
 * this limitation.
 * </p>
 * <p>
 * Also note that secure preferences only intended to store relatively small
 * size data, such as passwords. If you need to securely store large objects,
 * consider encrypting such objects in a symmetric way using randomly generated
 * password and use secure preferences to store the password.
 * </p>
 * <p>
 * If secure preferences were modified, the framework will automatically save
 * them on shutdown.
 * </p>
 * <p>
...
public interface ISecurePreferences {
	 ...
	 *            value
	 * @return value associated the key. If value was stored in an encrypted form,
	 ...
	 * <p>
	 * If the node path is invalid, an {@link IllegalArgumentException} will be
	 * thrown by this method. The valid node path:
	 * </p>
	 * <ul>
	 * <li>contains only ASCII characters between 32 and 126 (ASCII alphanumeric and
	 * printable characters);</li>
	...
	/**
	 * Checks if the node corresponding to the specified path exists in this tree of
	 * secure preferences.
	 * <p>
	 * If the node path is invalid, an {@link IllegalArgumentException} will be
	 ...
	 * @see org.osgi.service.prefs.Preferences#node(String)
	 * @param pathName absolute or relative path to the node
	 * @return <code>true</code> if node corresponding to the path exists,

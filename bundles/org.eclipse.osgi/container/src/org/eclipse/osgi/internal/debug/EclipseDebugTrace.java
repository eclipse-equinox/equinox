...
class EclipseDebugTrace implements DebugTrace {
	...
	/**
	 * The system property used to specify size a trace file can grow before it is
	 * rotated
	 */
	private static final String PROP_TRACE_SIZE_MAX = "eclipse.trace.size.max"; //$NON-NLS-1$
	/**
	 * The system property used to specify the maximum number of backup trace files
	 * to use
	 */
	...
	/**
	 * The date attribute written to the header of the trace file to show when this
	 * file was created
	 */
	...
	/**
	 * The delimiter used to separate trace elements such as the time stamp,
	 * message, etc
	 */
	private final static String TRACE_ELEMENT_DELIMITER = "|"; //$NON-NLS-1$
	/**
	 * The string written in place of the
	 * {@link EclipseDebugTrace#TRACE_TRACE_ELEMENT_DELIMITER} in entries
	 */
	...
	/**
	 * An optional argument to specify the name of the class used by clients to
	 * trace messages. If no trace class is specified then the class calling this
	 * API is assumed to be the class being traced.
	 */
	private String traceClass = null;
	/** The symbolic name of the bundle being traced */
	private String bundleSymbolicName = null;
	/**
	 * DebugOptions are used to determine if the specified bundle symbolic name +
	 * option-path has debugging enabled
	 */
	...
	/**
	 * Construct a new EclipseDebugTrace for the specified bundle symbolic name and
	 * write messages to the specified trace file.
	 *
	 * @param bundleSymbolicName The symbolic name of the bundle being traced
	 * @param debugOptions       Used to determine if the specified bundle symbolic
	 *                           name + option-path has tracing enabled
	 * @param traceClass         The class that the client is using to perform trace
	 *                           API calls
	 */
	EclipseDebugTrace(final String bundleSymbolicName, final FrameworkDebugOptions debugOptions,
			final Class<?> traceClass) {
		this.consoleLog = "true" //$NON-NLS-1$
				.equals(debugOptions.getConfiguration().getConfiguration(EclipseStarter.PROP_CONSOLE_LOG));
	 ...
	 * @return Returns true if debugging is enabled for the specified option-path on
	...
	public void trace(final String optionPath, final String message) {
		...
		if (isDebuggingEnabled(optionPath)) {
			final FrameworkDebugTraceEntry record = new FrameworkDebugTraceEntry(bundleSymbolicName, optionPath,
					message, traceClass);
			writeRecord(record);
		}
	}
	...
	public void traceDumpStack(final String optionPath) {
		...
		if (isDebuggingEnabled(optionPath)) {
			...
			// the second element in this stack trace is going to either be the caller or
			// the trace class. Ignore it only if a traceClass is defined
			...
			final FrameworkDebugTraceEntry record = new FrameworkDebugTraceEntry(bundleSymbolicName, optionPath,
					messageBuffer.toString(), traceClass);
			writeRecord(record);
		}
	}
	...
	/**
	 * Utility method to convert an array of StackTraceElement objects to form a
	 * String representation of a stack dump
	 *
	 * @param elements The array of StackTraceElement objects
	...
	/**
	 * Checks the trace file size. If the file size reaches the limit then the trace
	 * file is rotated.
	 *
	 * @param traceFile The tracing file
	 * @param timestamp the timestamp for the session; this is the same timestamp as
	 *                  the first entry
			...
			if ((traceFile != null) && traceFile.exists()) {
				if ((traceFile.length() >> 10) > maxTraceFileSize) { // Use KB as file size unit.
					...
					if (traceFileName.toLowerCase().endsWith(TRACE_FILE_EXTENSION)) {
						backupFilename = traceFileName.substring(0,
								traceFileName.length() - TRACE_FILE_EXTENSION.length()) + BACKUP_MARK
								+ backupTraceFileIndex + TRACE_FILE_EXTENSION;
					} else {
					...
					if (backupFile.exists()) {
						if (!backupFile.delete()) {
							...
							if (backupFile
									.renameTo(new File(backupFile.getAbsolutePath() + System.currentTimeMillis()))) {
					...
					/*
					 * Write a header to new log file stating that this new file is a continuation
					 * file. This method should already be called with the file lock set so we
					 * should be safe to update it here.
					 */
					Writer traceWriter = null;
					try {
						traceWriter = openWriter(traceFile);
						writeComment(traceWriter,
								"This is a continuation of trace file " + backupFile.getAbsolutePath()); //$NON-NLS-1$
						writeComment(traceWriter,
								EclipseDebugTrace.TRACE_FILE_VERSION_COMMENT + EclipseDebugTrace.TRACE_FILE_VERSION);
						writeComment(traceWriter,
								EclipseDebugTrace.TRACE_FILE_VERBOSE_COMMENT + debugOptions.isVerbose());
	 ...
	 *
	 * @return A formatted time stamp based on the
	 *         {@link EclipseDebugTrace#TRACE_FILE_DATE_FORMATTER} formatter
	 */
	...
	/**
	 * Accessor to retrieve the text of a {@link Throwable} in a formatted manner so
	 * that it can be written to the trace file.
	 *
	 * @param error The {@lnk Throwable} to format
	 * @return The complete text of a {@link Throwable} as a {@link String} or null
	 *         if the input error is null.
	 */
	 ...
	 *
	 * @param traceWriter the trace writer
	 * @param timestamp   the timestamp for the session; this is the same timestamp
	 *                    as the first entry
	...
	private void writeMessage(final Writer traceWriter, final FrameworkDebugTraceEntry entry) throws IOException {
		...
		message.append(" "); //$NON-NLS-1$
		if (!debugOptions.isVerbose()) {
			// format the trace entry for quiet tracing: only the thread name, timestamp,
			// trace message, and exception (if necessary)
	...
	/**
	 * Encodes the specified string to replace any occurrence of the
	 * {@link EclipseDebugTrace#TRACE_ELEMENT_DELIMITER} string with the
	 * {@link EclipseDebugTrace#TRACE_ELEMENT_DELIMITER_ENCODED} string. This can be
	 * used to ensure that the delimiter character does not break parsing when the
	 * entry text contains the delimiter character.
	 *
	 ...
	 *         {@link EclipseDebugTrace#TRACE_ELEMENT_DELIMITER_ENCODED}. A
	 *         <code>null</code> value will be returned if the input string is
	 *         <code>null</code>.
	 */
	...
	/**
	 * Creates the trace writer. If the tracing file is null then the writer will
	 * use System.out to print any messages.
	 *
	 * @param traceFile The tracing file
	...
	private Writer openWriter(final File traceFile) {
		...
		if (traceFile != null) {
			try {
				...
				// ignore and fall back to system.out; but print error message to indicate what
				// happened

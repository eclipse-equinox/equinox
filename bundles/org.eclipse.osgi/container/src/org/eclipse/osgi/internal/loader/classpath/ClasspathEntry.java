...
public class ClasspathEntry {
	...
	public ClasspathEntry(BundleFile bundlefile, ProtectionDomain domain, Generation generation) {
		...
		if (manifest != null && generation.getBundleInfo().getStorage().getConfiguration().DEFINE_PACKAGE_ATTRIBUTES) {
			...
			perPackageManifestAttributes = manifestPackageAttributesMapFor(manifest.getEntries().entrySet(),
					mainManifestPackageAttributes);
		} else {
	...
	private static ManifestPackageAttributes manifestPackageAttributesFor(Attributes attributes,
	...
	private static Map<String, ManifestPackageAttributes> manifestPackageAttributesMapFor(
			Set<Entry<String, Attributes>> entries, ManifestPackageAttributes defaultAttributes) {
	...
	/**
	 * Adds the BundleFile objects for this classpath in the proper order for
	 * searching for resources. This handles Multi-Release ordering also.
	 */
	...
	ManifestPackageAttributes manifestPackageAttributesFor(String packageName) {
		ManifestPackageAttributes perPackage = perPackageManifestAttributes == null ? null
				: perPackageManifestAttributes.get(packageName);
		if (perPackage != null) {
			return perPackage;
		}

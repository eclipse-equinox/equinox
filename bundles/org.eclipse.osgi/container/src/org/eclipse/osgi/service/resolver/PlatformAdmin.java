 ...
 * packages known to the Framework. The PlatformAdmin service also allows
 * bundles with sufficient privileges to update the state of the framework by
 * committing a new configuration of bundles and packages.
 *
...
public interface PlatformAdmin {
	 ...
	 * changes to the returned state, a mutable state must be requested. Otherwise,
	 * an immutable state should be requested. In this case, invoking any of the
	 * operations that could cause the state to be changed will throw an
	 * <code>java.lang.UnsupportedOperationException</code>.
	 * <p>
	 * If a mutable state is requested, the resulting state will
	...
	/**
	 * Commit the differences between the current state and the given state. The
	 * given state must return true from State.isResolved() or an exception is
	 * thrown. The resolved state is committed verbatim, as-is.
	 *
	 * @param state the future state of the framework
	 * @throws BundleException if the id of the given state does not match that of
	 *                         the current state or if the given state is not
	 *                         resolved.
	 */
	 ...
	 *                                  in the state managed by this platform admin.
	 * @since 3.4
	 */

...
public interface SignedContent {
	...
	/**
	 * Returns all entries of the content. The returned entries can be used to
	 * verify the entry content using {@link SignedContentEntry#verify()} and get
	 * signer info for each entry in this content using
	 * {@link SignedContentEntry#getSignerInfos()}. Note that this operation may be
	 * expensive because it requires an exhaustive search for entries over the
	 * entire content.
	 * <p>
	 * Unsigned entries are included in the result. Entries for which signer info
	 * exists but no content is found are also returned. For example, when an entry
	 * is removed from a signed jar but the jar is not resigned, the signer thinks
	 * the entry should exist but the content got removed. This would be considered
	 * an invalid entry which would fail verification.
	 * </p>
	 * 
	...
	/**
	 * Returns the signing time for the signer info. If no TSA signers exist then
	 * null is returned
	 * 
	...
	/**
	 * Checks if the certificates are valid for the specified signer. If the signer
	 * has a singing time returned by {@link #getSigningTime(SignerInfo)} then that
	 * time is used to check the validity of the certificates; otherwise the current
	 * time is used.
	 * 
	 * @param signerInfo the signer info to check validity for.
	 * @throws CertificateExpiredException     if one of the certificates of this
	 *                                         signer is expired
	 * @throws CertificateNotYetValidException if one of the certificates of this
	 *                                         signer is not yet valid
	 */
